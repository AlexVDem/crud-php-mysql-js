...
class w2grid_class {
    ...
    public function saveChanges($table, $keyField, $data) {
        global $db;
        ...
        if ($data['recid'] == '' || $data['recid'] == '0') {
            ...
            $values = "";
            foreach ($data as $k => $v) {
                if ($k == $keyField || $k == 'recid') continue; // key field should not be here
                if ($values != '') $values .= ", ";
                if (substr($v, 0, 2) == "__") {
                    $values .= addslashes($k)." = ".addslashes(substr($v, 2));
                } else {
    ...
    public function newRecord($table, $data) {
        ...
        //$sql = "INSERT INTO $table($fields)  DEFAULT VALUES ";
